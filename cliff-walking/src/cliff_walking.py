        ...
        # use Expected SARSA
            ...
            for action_ in actions:
                if action_ in greedy_actions:
                    ...
                    target += (((1.0 - exploration_probability) / len(greedy_actions) + exploration_probability / len(actions))
                    * action_value_estimates[next_state[0], next_state[1], action_])
                else:
                    # non-greedy action selection
                    target += exploration_probability / len(actions) * action_value_estimates[next_state[0], next_state[1], action_]
...
def q_learning(action_value_estimates, step_size=step_size):
    ...
    while state != goal:
        ...
        action_value_estimates[state[0], state[1], action] += step_size * (reward + discount * np.max(action_value_estimates[next_state[0], next_state[1], :])
