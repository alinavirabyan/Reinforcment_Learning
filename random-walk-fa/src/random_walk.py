...
def gradient_monte_carlo(value_function, step_size, states_distribution=None):
    ...
    for state in trajectory[:-1]:
        ...
        update = step_size * (reward - value_function.value(state))

        # update VF parameters
        value_function.update(update, state)
...
def semi_gradient_temporal_difference(value_function, steps_number, step_size):
    ...
    while True:
        ...
        if update_time >= 0:
            ...
            state_to_be_updated = states[update_time]

            # if the state to be updated is not a terminal state
            if not state_to_be_updated in terminal_states:
                # calculate the update size
                update_size = step_size * (returns - value_function.value(state_to_be_updated))

                # update the VF
                value_function.update(update_size, state_to_be_updated)
